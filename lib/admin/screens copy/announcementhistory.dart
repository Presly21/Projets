  ...
  final List<Map<String, dynamic>> announcements = List.generate(
    ...
    (index) => {
      ...
      'file': index % 2 == 0
          ...
          : File('path/to/sample.jpg'), // Exemple de chemins de fichier
    },
  );
...
class AnnouncementDetailPage extends StatelessWidget {
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: <Widget>[
              ...
              const SizedBox(height: 10),
              Text('Visibilité: ${announcement['visibility']}',
                  style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
              Text('Épinglé: ${announcement['pinned'] ? 'Oui' : 'Non'}',
                  style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
              Text('Département: ${announcement['department']}',
                  style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 10),
              Text('Date de Création: ${announcement['createdAt']}',
                  style: const TextStyle(fontSize: 16)),
              const SizedBox(height: 20),
              if (announcement['file'] != null)
                _buildFilePreview(announcement['file'], context),
            ],
          ),
        ),
      ),
  ...
  Widget _buildFilePreview(File file, BuildContext context) {
    return Stack(
          ...
          onSelected: (value) {
            switch (value) {
              case 'Télécharger':
                ...
                break;
              case 'Imprimer':
                _printFile(file.path);
                break;
            }
          },
          itemBuilder: (BuildContext context) {
            return ['Télécharger', 'Imprimer',]
                .map((String choice) {
              return PopupMenuItem<String>(
                value: choice,
                child: Text(choice),
  ...
  Future<void> _downloadFile(String filePath) async {
    ...
    print('Télécharger le fichier : $filePath');
  }
