...
class PaySlipController extends GetxController {
  ...
  RxList<PaySlipModelList> totalPaySlipList = RxList<PaySlipModelList>();
  RxList<PaySlipModelList> paySlipList = RxList<PaySlipModelList>();
  ...
  Future<void> fetchingPaySlipList() async {
    try {
  ...
  if (error is BadRequestException) {
    var apiError = json.decode(error.message ?? '{}'); // Handle null safely
    CustomDialogBox.showErrorDialog(description: apiError["reason"] ?? "Unknown error");
  } else {
  ...
  PaySlipModelList getFilteredPaySlip() {
    ...
    if (filteredPaySlips.isEmpty) {
      return PaySlipModelList(); // Return an empty PaySlipModelList if no matching pay slip is found
    }
